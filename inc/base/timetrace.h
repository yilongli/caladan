...
// FIXME: a per-core tt_buffer might not be a good fit for Caladan: iokernel
// and runtime need different mechanisms to get the current cpu id, so a
// per-core model won't be general enough to be put under inc/base.

// FIXME: but wait, w/o tt_buffers[], how can we merge all timetraces? if we
// keep tt_buffers[], how to define the meaning of the index? runtime & app
// will treat it as cpu id, iokernel will treat it as kthread id? (if iokernel
// ever becomes multithreaded)

extern struct tt_buffer *tt_buffers[];
extern unsigned int nr_tt_buffers;
